...
class DBManager {
    ...
    public:
      ...
      bool insertTopic(int id, const std::string& title, const std::string& url) {
    char query[512];

    // Insert with id
    snprintf(query, sizeof(query),
             "INSERT INTO topics (id, title, url) VALUES (%d, '%s', '%s')",
             id, title.c_str(), url.c_str());
    ...
    if (mysql_affected_rows(conn) == 0) {
        std::cout << "ID not found. No topic updated.\n";
        return false;
    }
    ...
    if (mysql_affected_rows(conn) == 0) {
        std::cout << "ID not found. No topic deleted.\n";
        return false;
    }
...
int main(){
    ...
    switch(choice) {
        ...
        case 3: {
            ...
            std::cout << "Enter URL: ";
            std::cin >> url;
            if (ptrDbManager->insertTopic(id,title, url)) {
